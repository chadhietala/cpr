...
var createDirs = function(buffer, to, options, callback) {
    var stack = new Stack();
    var dir = buffer.next();
    var stat;

    while(dir) {
      stat = options.stats[dir];
      var toHash = options.toHash[dir];

      console.log(dir, toHash);
        if (toHash && typeof toHash === 'string') {
            fs.stat(toHash, stack.add(function(err, s) { /* jshint ignore:line */
            if (s && !s.isDirectory()) {
                options.errors.push(new Error(toHash + ' exists and is not a directory, can not create'));
            } else {
                mkdirp(toHash, stat.mode, stack.add(function(err) {
                    /*istanbul ignore next*/
                    if (err) {
...
var createBuffers = function(bufferSize, dirs) {
  bufferSize = bufferSize || 30;
  var i = 0;
  var buffers = [];
  var segment = [];

  while (i < 30 && dirs.length > 0) {
    segment.push(dirs[i]);
    if (i === 29) {
      buffers.push(new Buffer(segment));
      segment = [];
      i = 0;
    } else {
      i++;
...
var bufferedCreation = function(dirs, files, options, callback) {
  var buffers = createBuffers(options.bufferSize, dirs);

  buffers.forEach(function(buffer) {
    createDirs(buffer, options, function () {
      createFiles(files, options.to, options, function() {
          var out = [], err;
        ...
        getTree(options.from, options, function(err, tree) {
            filterTree(tree, options, function(err, t) {
                splitTree(t, options, function(dirs, files) {
                    ...
                    bufferedCreation(dirs, files, options, callback);
                });
            });
        });
    };
